...
func TestConcurrentOne(t *testing.T) {
	...
	// Create a channel to store the errors from the goroutines
	errs := make(chan error, nrpcs)
	...
	for ii := 0; ii < nrpcs; ii++ {
		go func(i int) {
			...
			if reply != wanted {
				errs <- fmt.Errorf("wrong reply %v from Handler2, expecting %v", reply, wanted)
			} else {
				errs <- nil
			}
	...
	// Wait for all the goroutines to finish and then check for errors
	for i := 0; i < nrpcs; i++ {
		err := <-errs
		if err != nil {
			t.Fatal(err)
